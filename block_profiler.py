...
def classify_from_logs(logs) -> Tuple[str, Optional[str]]:
    ...
    for lg in logs:
        ...
        t0 = topics[0]
        if isinstance(t0, (bytes, bytearray, HexBytes)): t0 = Web3.to_hex(t0)
        t0 = t0.lower()
        addr = lg.get("address", "").lower()
        if t0 == SIG_TRANSFER.lower():
            if lg.get("data") and lg["data"] != "0x": erc20_count += 1
            else: erc721_count += 1
            if addr: tokens[addr] += 1
        elif t0 in (SIG_ERC1155_SINGLE.lower(), SIG_ERC1155_BATCH.lower()):
            erc1155_count += 1
            if addr: tokens[addr] += 1
    dominant = tokens.most_common(1)[0][0] if tokens else None
    if erc20_count > 0 and erc721_count == 0 and erc1155_count == 0: return "erc20_transfer", dominant
    if erc721_count > 0 and erc20_count == 0 and erc1155_count == 0: return "erc721_transfer", dominant
    if erc1155_count > 0 and erc20_count == 0 and erc721_count == 0: return "erc1155_transfer", dominant
    if erc20_count or erc721_count or erc1155_count: return "mixed_token_activity", dominant
    ...
    try:
        ...
        for tr in traces:
            if tr.get("type") == "call":
                ...
                if isinstance(val_hex, str) and val_hex.startswith("0x"): total += int(val_hex, 16)
        return total
    except Exception:
        return 0
...
def trace_internal_value_geth(w3: Web3, block_number: int) -> int:
    try:
        ...
        def walk(node):
            nonlocal total
            val = node.get("value")
            if isinstance(val, str) and val.startswith("0x"): total += int(val, 16)
...
def profile_chunk(w3: Web3, start: int, end: int, *, skip_contract_check: bool, tx_cap: Optional[int], receipt_workers: int, trace_mode: str, csv_writer=None) -> Dict[str, Any]:
    ...
    DEFAULT_TYPES = ["eth_transfer","contract_creation","erc20_transfer","erc721_transfer","erc1155_transfer","other_contract_call","mixed_token_activity","other_eoa_call"]
    contract_cache: Dict[str, bool] = {}

    def fetch_receipt(tx_hash): return w3.eth.get_transaction_receipt(tx_hash)
            ...
            for fut in as_completed(futures):
                ...
                if tx.get("to") is None:
                    ...
                    if tx_type == "other_contract_call":
                        if value_wei > 0: tx_type = "eth_transfer"
                        elif not skip_contract_check:
                            addr = tx["to"].lower()
                            if addr not in contract_cache: contract_cache[addr] = is_contract(w3, addr)
                            if not contract_cache[addr]: tx_type = "other_eoa_call"

                stats = tx_type_stats[tx_type]
                stats.count += 1; stats.gas_used += gas_used; stats.gas_price_wei_sum += gas_price_wei; stats.eth_value_wei_sum += value_wei

                block_counts[tx_type] += 1; total_eth_wei += value_wei

                if tx.get("to"): top_contracts[tx["to"].lower()] += 1
                if token_or_contract: top_tokens[token_or_contract.lower()] += 1

                if tx_cap is not None and total_tx >= tx_cap: break

        if csv_writer:
            row = [block["number"], block["timestamp"], len(txs)]
            for t in DEFAULT_TYPES: row.append(block_counts.get(t, 0))
            row.extend([block.get("gasUsed", 0) if "gasUsed" in block else block_gas_used_sum, block.get("gasLimit", 0)])
    ...
    if trace_mode and trace_mode != "none":
        for bnum in range(start, end + 1):
            if trace_mode == "erigon": internal_total += trace_internal_value_erigon(w3, bnum)
            elif trace_mode == "geth": internal_total += trace_internal_value_geth(w3, bnum)

    summary = {
        "start_block": start, "end_block": end, "block_count": end - start + 1, "total_tx": total_tx,
        "unique_senders": len(unique_from), "unique_receivers": len(unique_to),
        "total_eth_transferred_eth": float(wei_to_eth(total_eth_wei)), "total_internal_value_eth": float(wei_to_eth(internal_total)),
        "tx_types": {
            k: {
                "count": v.count,
                "gas_used": v.gas_used,
                "avg_gas_price_gwei": float((Decimal(v.gas_price_wei_sum) / Decimal(max(v.count, 1)) / Decimal(1e9)).quantize(Decimal("0.0001"))) if v.count else 0.0,
                "eth_value_sum_eth": float(wei_to_eth(v.eth_value_wei_sum)),
            } for k, v in sorted(tx_type_stats.items(), key=lambda kv: kv[0])
        },
        "top_contracts_by_tx": Counter(top_contracts).most_common(20), "top_tokens_by_events": Counter(top_tokens).most_common(20),
    }
    return summary

def profile_range(rpc: str, start: int, end: int, *, out: str, csv_path: Optional[str], skip_contract_check: bool, tx_cap: Optional[int], concurrency: int, chunk_size: int, trace_mode: str) -> Dict[str, Any]:
    if end < start: raise ValueError("--end must be >= --start")
    w3 = Web3(Web3.HTTPProvider(rpc, request_kwargs={"timeout": 60}))
    if not w3.is_connected(): raise SystemExit("Could not connect to RPC endpoint.")
    chain_id = w3.eth.chain_id

    csv_writer = None; csv_file = None
    if csv_path:
        csv_file = open(csv_path, "w", newline=""); csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["block_number","timestamp","tx_count","eth_transfer","contract_creation","erc20_transfer","erc721_transfer","erc1155_transfer","other_contract_call","mixed_token_activity","other_eoa_call","block_gas_used","block_gas_limit"])

    overall = {"start_block": start, "end_block": end, "chain_id": chain_id, "block_count": end - start + 1,
               "total_tx": 0, "unique_senders": -1, "unique_receivers": -1,
               "total_eth_transferred_eth": 0.0, "total_internal_value_eth": 0.0,
               "tx_types": {}, "top_contracts_by_tx": [], "top_tokens_by_events": [],
               "limits": {"tx_cap": tx_cap, "skip_contract_check": skip_contract_check, "concurrency": concurrency, "chunk_size": chunk_size, "trace_mode": trace_mode},
               "notes": ["ERC20 vs ERC721 from Transfer payload (amount vs none).", "Mixed token activity indicates multiple token standards in a single tx.", "ETH transferred sums only tx.value; internal transfers via traces (optional)."]}
    ...
    while cur <= end:
        ...
        chunk = profile_chunk(w3, cur, chunk_end, skip_contract_check=skip_contract_check, tx_cap=tx_cap, receipt_workers=concurrency, trace_mode=trace_mode, csv_writer=csv_writer)
        total_tx_acc += chunk["total_tx"]
        total_eth_wei_acc += int(D(str(chunk["total_eth_transferred_eth"])) * D(10**18))
        internal_wei_acc += int(D(str(chunk["total_internal_value_eth"])) * D(10**18))
        for k, v in chunk["tx_types"].items():
            s = tx_types_agg[k]; s.count += v["count"]; s.gas_used += v["gas_used"]
            s.gas_price_wei_sum += int(D(str(v["avg_gas_price_gwei"])) * D(1e9) * D(v["count"]))
            s.eth_value_wei_sum += int(D(str(v["eth_value_sum_eth"])) * D(10**18))
        for addr, c in chunk["top_contracts_by_tx"]: top_contracts[addr] += c
        for addr, c in chunk["top_tokens_by_events"]: top_tokens[addr] += c
        pbar.update(chunk_end - cur + 1); cur = chunk_end + 1
    pbar.close()

    overall["total_tx"] = total_tx_acc
    overall["total_eth_transferred_eth"] = float(wei_to_eth(total_eth_wei_acc))
    overall["total_internal_value_eth"] = float(wei_to_eth(internal_wei_acc))
    overall["tx_types"] = {k: {"count": v.count, "gas_used": v.gas_used, "avg_gas_price_gwei": float((Decimal(v.gas_price_wei_sum) / Decimal(max(v.count, 1)) / Decimal(1e9)).quantize(Decimal("0.0001"))) if v.count else 0.0, "eth_value_sum_eth": float(wei_to_eth(v.eth_value_wei_sum))} for k, v in sorted(tx_types_agg.items(), key=lambda kv: kv[0])}
    overall["top_contracts_by_tx"] = top_contracts.most_common(20); overall["top_tokens_by_events"] = top_tokens.most_common(20)
    if csv_file: csv_file.close()
    with open(out, "w") as f: json.dump(overall, f, indent=2)
    return overall
...
def main():
    ...
    summary = profile_range(rpc=args.rpc, start=args.start, end=args.end, out=args.out, csv_path=args.csv, skip_contract_check=args.skip_contract_check, tx_cap=args.tx_cap, concurrency=args.concurrency, chunk_size=args.chunk_size, trace_mode=args.trace)
    print(f"Wrote JSON summary to {args.out}")
    if args.csv: print(f"Wrote per-block CSV to {args.csv}")
